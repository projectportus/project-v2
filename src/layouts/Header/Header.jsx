      ...
      <div className='bg-white flex justify-around items-center h-24 max-[400px]:justify-around md:gap-5 md:justify-around'>
          <div className='logoBox flex justify-center w-3xs '>
            <Link to="/" className="w-full h-full">
              <img src={logo} className='w-45 sm:w-50 md:w-56 lg:w60 xl:w-60' alt="logo" />
            </Link>
          </div>

            <div className="links md:text-[20px] hidden lg:flex justify-between items-center w-107 ">
              <Link to="/" className="link-items font-bold">Home</Link>
              <Link to="/shop" className="link-items font-semibold">Shop</Link>
              <a href="#" className="link-items font-semibold">About</a>
              <Link to="/contact-us" className="link-items font-semibold">Contact Us</Link>
          ...
          <button onClick={() => setOpen(!open)} className='z-50 burger-menu lg:hidden justify-center items-center sm:flex min-[400px]:mr-[2%]'>
          ...
          <div
            className={`fixed top-0 left-0  xl:hidden w-full bg-[#F9F1E7] flex flex-col items-center z-50 gap-2
              transform transition-all duration-500 ${open ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'}`}>
            <div className='flex w-full justify-between items-end mt-2'>
              <div className="logo  w-fit flex justify-start mt-[20px] ml-[20px] ">
                <Link to="/" className="link-items w-full">
                  <img src={logo} className='w-45 sm:w-50 md:w-56 lg:w-60 xl:w-60' alt="logo" />
                </Link>
              </div>
              <button onClick={() => setOpen(!open)} className=' z-51 burger-menu lg:hidden justify-center items-center sm:flex min-[400px]:mr-[2%]'>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-12">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
                </svg>
              </button>
            ...
            <div className='flex items-center justify-around h-[10vh]  w-full'>
              ...
              <Link to="/shop" className="link-items font-semibold"> {icons.search}</Link>
              <Link to="/about" className="link-items"> {icons.heart} </Link>
